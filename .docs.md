...
The system uses two primary thresholds to determine drowsiness states based on yawning frequency and eye closure duration.
...
def analyze_drowsiness(yawn_count, eye_closed_frames):
    is_drowsy_yawn = yawn_count >= DROWSINESS_THRESHOLD_YAWN
    is_drowsy_eyes = eye_closed_frames >= DROWSINESS_THRESHOLD_EYE_CLOSED
    
    return is_drowsy_yawn or is_drowsy_eyes
```
...
3. Adjust based on:
   ...
   - Safety requirements
4. Consider relationships between parameters:
   - Higher yawn threshold might need lower eye closure threshold
   - Adjust both metrics based on monitoring duration
   - Balance sensitivity with specificity

#### Use Cases
- **Long-haul driving:**
  - Higher `DROWSINESS_THRESHOLD_YAWN` (7-8)
  - Higher `DROWSINESS_THRESHOLD_EYE_CLOSED` (40-45)
- **Safety-critical operations:**
  - Lower `DROWSINESS_THRESHOLD_YAWN` (4-5)
  - Lower `DROWSINESS_THRESHOLD_EYE_CLOSED` (30-35)
- **Urban delivery:**
  - Medium `DROWSINESS_THRESHOLD_YAWN` (5-6)
  - Medium `DROWSINESS_THRESHOLD_EYE_CLOSED` (35-40)
